...
namespace EnchantedCoder.Data.EntityFrameworkCore.CodeGenerator.Tool
{
	public class Program
	{
		...
		{
			Console.WriteLine("Enchanted Coder Entity Framework Core CodeGenerator Tool");
			Console.WriteLine("----------------------------------------------");
			...
			FileInfo[] files = dataLayerBinDirectory
				...
				.Where(file => !file.Name.EndsWith("EnchantedCoder.Entity.dll"))
			...
			try
			{
				assembly = assemblyLoadContext.LoadFromAssemblyName(new AssemblyName { Name = "EnchantedCoder.Data.EntityFrameworkCore.CodeGenerator" });
			}
			catch (Exception ex)
			{
				Console.WriteLine("Failed to load EnchantedCoder.Data.EntityFrameworkCore.CodeGenerator");
			...
			if (assembly == null)
			{
				Console.WriteLine("Assembly EnchantedCoder.Data.EntityFrameworkCore.CodeGenerator could not be loaded.");
				return;
			}

			Type program = assembly.GetType("EnchantedCoder.Data.EntityFrameworkCore.CodeGenerator.Program");
			if (program == null)
			{
