using EnchantedCoder.Services.Ares;
using Microsoft.VisualStudio.TestTools.UnitTesting;

namespace EnchantedCoder.Services.Tests.Ares
{
	[TestClass]
	public class AresServicesTests
	{
		...
		//[TestMethod]
		[TestCategory("Integration")]
		public void AresStandardService_EnchantedCoder()
		{
			const string EnchantedCoder = "";
			AresStandardService service = new AresStandardService();
			var result = service.GetData(EnchantedCoder);
			Assert.AreEqual(1, result.Data.Count);
			Assert.IsFalse(result.PrilisMnohoVysledku);
			Assert.AreEqual("25612697", result.Data[0].Ico);
			Assert.AreEqual(EnchantedCoder, result.Data[0].Nazev);
		}
