...
namespace EnchantedCoder.Data.EntityFrameworkCore.BusinessLayer.Metadata.Conventions
{
	...
	public class LocalizationTablesParentEntitiesConvention : IForeignKeyAddedConvention, IForeignKeyPropertiesChangedConvention
	{
		...
		private void TrySetColumnName(IConventionForeignKey foreignKey)
		{
			...
			if (!entityType.ClrType.GetInterfaces().Any(itype => itype.IsGenericType && itype.GetGenericTypeDefinition() == typeof(EnchantedCoder.Model.Localizations.ILocalization<,>)))
			{
				return;
			}
